...
cv_ef_defol <- defoliate_trees(host_tree = sub_cv_abba, nonhost_chron = sub_cv_pcru, duration_years = 8, list_output = FALSE)
...
Year<-sub_cv_year_abba
RWI<-cv_ef_comp$mean_gsi
...
cv_gsi_p<-ggplot(data=dfi, aes(x=Year, y=RWI))+
  geom_area(aes(fill = Mean))+
  geom_hline(yintercept = 1)+
  scale_y_continuous(trans=t_shift)+
  ggtitle("Canaan Valley Region Growth Suppression Index")+
